...
flowchart LR
  ...
  subgraph Microservices
    ...
    subgraph AuthSvc["Auth Service"]
      direction TB
      A1[Presentation Layer<br/>(REST/gRPC Endpoints)]
      A2[Business Logic Layer<br/>(OAuth2, JWT-Handling)]
      A3[Data Access Layer<br/>(ORM / Repository)]
      A3 --> AuthDB[(Auth-DB)]
    end
    ...
    subgraph UserSvc["User Service"]
      direction TB
      U1[Presentation Layer<br/>(User-API)]
      U2[Business Logic Layer<br/>(Profile, Rollen)]
    ...
    subgraph CourseSvc["Course Service"]
      direction TB
      C1[Presentation Layer<br/>(Course-API)]
      C2[Business Logic Layer<br/>(Kurs-CRUD)]
    ...
    subgraph MaterialSvc["Material Service"]
      direction TB
      M1[Presentation Layer<br/>(Material-API)]
      M2[Business Logic Layer<br/>(Upload, Versioning)]
    ...
    subgraph EnrollmentSvc["Enrollment Service"]
      direction TB
      E1[Presentation Layer<br/>(Enrollment-API)]
      E2[Business Logic Layer<br/>(An- und Abmeldung)]
    ...
    subgraph ExamSvc["Exam Service"]
      direction TB
      X1[Presentation Layer<br/>(Exam-API)]
      X2[Business Logic Layer<br/>(Prüfungs-Setup)]
    ...
    subgraph Submits["Submission & Scoring & Grading"]
      direction TB
      S1[Presentation Layer<br/>(Submission-API)]
      S2[Business Logic Layer<br/>(Speicherung)]
      S3[Data Access Layer]
      S3 --> SubmDB[(Submission-DB)]

      SC1[Presentation Layer<br/>(Scoring-API)]
      SC2[Business Logic Layer<br/>(Automatisierte Bewertung)]
      SC3[Data Access Layer]
      SC3 --> ScoreDB[(Scoring-DB)]

      G1[Presentation Layer<br/>(Grading-API)]
      G2[Business Logic Layer<br/>(Manuelle Bewertung)]
    ...
    subgraph NotifSvc["Notification Service"]
      direction TB
      N1[Presentation Layer<br/>(Notify-API)]
      N2[Business Logic Layer<br/>(E-Mail, Push)]
